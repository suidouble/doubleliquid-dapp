                    ...
                    <q-tab-panels v-model="selectedTabInDialog" >
                        <q-tab-panel name="stakedSui" class="q-pa-xs">
                            ...
                            <template v-for="(staked_sui) in detailsForRowPopup.staked_staked_suis" v-bind:key="staked_sui.stake_activation_epoch">
                                ...
                                <br/> sorted ok 
                                    <q-icon name="check_circle" color="positive" v-if="staked_sui.diff5epochsGrowing >= 0" />
                                    <q-icon name="report" color="negative" v-if="staked_sui.diff5epochsGrowing < 0" />
                                    <span class="text-positive" v-if="staked_sui.diff5epochsGrowing > 0">+{{ staked_sui.diff5epochsGrowing.toFixed(13) }}</span>
                                    <span class="text-negative" v-if="staked_sui.diff5epochsGrowing < 0">{{ staked_sui.diff5epochsGrowing.toFixed(13) }}</span>
                                <q-separator />
                            </template>
...
export default {
		...
		return {
            ...
            transactionsColumns: [
                ...
                },
                {
                    ...
                    field: row => {
                        ...
                        if (row.type == 'withdraw' || row.type == 'withdraw_fast') {
                            if (row.send > 1000) {
            ...
            epochColumns: [
                ...
                },
                {
                    ...
                    align: 'left',
                    field: row => row.price_of_last_transaction,
                    // format: val => `${val}`,
                    sortable: true
                },
                {
                ...
                },
                {
                    name: 'fee_pool_token',
                    required: true,
                    label: 'fee_pool_token',
                    align: 'left',
                    field: row => row.fee_pool_token,
                    // format: val => `${val}`,
                    sortable: true
                },
            ],
	...
	methods: {
        ...
        getRowDetails(row) {
            ...
            // calculate difference of diff5epochs with prev stakedSui
            for (let i = 0; i < ret.staked_staked_suis.length; i++) {
                let diffOfDiffs = 0;
                if (i > 0) {
                    diffOfDiffs = ret.staked_staked_suis[i].diff5epochs - ret.staked_staked_suis[i-1].diff5epochs;
                }
                ret.staked_staked_suis[i].diff5epochsGrowing = diffOfDiffs;
            }
